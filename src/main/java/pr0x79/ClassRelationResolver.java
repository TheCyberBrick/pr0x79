...
public class ClassRelationResolver {
	 ...
	 * @param traverser The traverse that traverses the superclasses. The traverse method immediately returns true when the traverser returns true. The first argument is the internal class name and the second argument is whether the class is an interface.
	 * @param traverseInterfaces Whether interfaces should be traversed too
	 */
	public boolean traverseHierarchy(String clsName, BiFunction<String, Boolean, Boolean> traverser, boolean traverseInterfaces) {
	...
	private boolean traverseHierarchy(ClassData cls, BiFunction<String, Boolean, Boolean> traverser, boolean traverseInterfaces) {
		String type = cls.name;
		ClassData info = cls;
		while (!"java/lang/Object".equals(type)) {
			if(traverser.apply(type, (info.access & Opcodes.ACC_INTERFACE) != 0)) {
				return true;
			}
		...
		return traverser.apply("java/lang/Object", false); //Visit Object at the end
	}
}
